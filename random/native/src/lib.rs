...
//! Export names match `#native` symbols; every exported `n_*` fn carries
//! `#[loft_native]`, so the build script emits a `loft_register_bridges_v1`
//! table and the interpreter dispatches through the generated uniform
//! marshal bridges.
//!
...
// @PLAN12 Phase 6r — the `loft_ffi::loft_register! { … }` invocation is
// GENERATED by `build.rs` (via `loft-ffi-build::generate_register_from_loft`)
// scanning this crate's loft sources for `#native` annotations.  Defining
// a native function IS registering it — no hand-maintained symbol list.
