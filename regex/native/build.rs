...
//! generates the register list — the SAME co-located annotations the compiler
//! binds against, so the register list cannot drift.  Bare `#native` →
//! `n_<fn>`.  `include!`d by `src/lib.rs`.
//!
//! Plan-74: the `n_*` impls carry `#[loft_native]`, so this emits the
//! `loft_register_bridges!` list too — the interpreter dispatches regex
