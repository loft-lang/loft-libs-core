...
/// the base64 input (local-dev: tracks loft-libs-core#13; the loft agent's vector<u8>
/// SHA fix may supersede this with a typed primitive).
#[loft_native]
#[unsafe(no_mangle)]
...
#[loft_native]
#[unsafe(no_mangle)]
pub unsafe extern "C" fn n_random_bytes(length: i64) -> LoftStr {
    // `i64`: the loft declaration types `length` as `integer`, and loft emits the extern
    // from that declaration, so it passes 64 bits.  Narrow inside the body, not in the
    // signature — a mismatched C signature is undefined behaviour, not a narrowing.
    cr_ret(random::bytes(length as i32))
}
