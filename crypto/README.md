...
(RFC 9180) key wrapping, **ChaCha20-Poly1305** / **XChaCha20-Poly1305** /
**AES-256-GCM** AEAD, **HKDF**, and an **OS CSPRNG**.  All pure-Rust (RustCrypto / dalek), no
...
| `chacha20poly1305_seal(key_b64, nonce_b64, aad_b64, pt_b64) -> text` | `ct‖tag`, base64 | key 32B, nonce 12B; **caller must keep nonces unique per key** (96-bit → random unsafe past ~2³² msgs) |
| `chacha20poly1305_open(key_b64, nonce_b64, aad_b64, ct_b64) -> text` | plaintext, base64 | `""` on auth failure |
| `xchacha20poly1305_seal(key_b64, nonce_b64, aad_b64, pt_b64) -> text` | `ct‖tag`, base64 | key 32B, **nonce 24B** — 192-bit nonce, so a fresh **random** nonce per message is safe (use for file/op bodies) |
| `xchacha20poly1305_open(key_b64, nonce_b64, aad_b64, ct_b64) -> text` | plaintext, base64 | `""` on auth failure |
